...
const POSTS_CACHE_TTL = 1e3 * 60;
async function getNotionPosts() {
  const cached = getCachedValue(POSTS_CACHE_KEY);
  if (cached) {
  ...
  const summaries = response.results.filter((page) => isFullPage(page)).map((page) => mapPageToSummary(page)).sort((a, b) => {
...
export { getNotionPost as a, getNotionPosts as g };
